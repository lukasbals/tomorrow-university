...
class TreeEnsembleProject:
    ...
    def run_baseline_comparison(self):
        ...
        for name, model in models.items():
            ...
            print(
                f"{name}: CV {cv_scores.mean():.3f}±{cv_scores.std():.3f}, Test {test_score:.3f}"
            )

    def select_best_model(self):
        """Identify best performing model"""
        best_cv_score = max(
            self.results.keys(), key=lambda k: self.results[k]["cv_mean"]
        )
    ...
    def generate_report(self):
        ...
        sorted_models = sorted(
            self.results.items(), key=lambda x: x[1]["cv_mean"], reverse=True
        )
        ...
        print(
            f"• Performance stability: {best_results['cv_std']:.3f} CV standard deviation"
        )
        print(
            f"• Generalization gap: {abs(best_results['cv_mean'] - best_results['test_score']):.3f}"
        )
